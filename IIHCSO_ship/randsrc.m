...
%
%   OUT = RANDSRC(M,N,ALPHABET,S) uses S to generate random noise samples
%   with the RAND function. S can be a random number stream specified by
%   RandStream. S can also be an integer, which seeds a random number
%   stream inside the RANDSRC function. If you want to generate repeatable
%   noise samples, then either reset the random stream input before calling
%   RANDSRC or use the same seed input.
%
%   Examples:
%
...
%
%   See also RAND, RANDI, RANDERR, RandStream.
...
for i=1:nargin
   ...
   elseif(isa(varargin{i},'RandStream'))
      ...
      error(message('comm:randsrc:InvalidArg'));
   end
end
...
switch sigStr
   ...
   otherwise
      error(message('comm:randsrc:InvalidSyntax'));
end
...
if (length(m) > 1) || (length(n) > 1)
   error(message('comm:randsrc:NonScalarMatrixDims'));
elseif (~isfinite(m)) || (~isfinite(n))
   error(message('comm:randsrc:InvalidMatrixDims'));
elseif (floor(m) ~= m) || (floor(n) ~= n) || (~isreal(m)) || (~isreal(n))
   error(message('comm:randsrc:NonIntegerMatrixDims'));
elseif (m < 0) || (n < 0)
   error(message('comm:randsrc:NonPositiveMatrixDims'));
end
...
if ar > 2
   error(message('comm:randsrc:InvalidAlphabet'));
...
elseif ar == 2
   ...
   if (~isreal(aprob)) || any(~isfinite(aprob)) || any(aprob < 0) ...
           || any(aprob > 1)
      error(message('comm:randsrc:InvalidProbabilityVal'));
   elseif (abs(sum(aprob) - 1) > sqrt(eps))
      error(message('comm:randsrc:InvalidProbabilitySum'));
   end
