function [new_velocity, new_position, new_sequence_pos,new_sequence_vel, flag] = inter_hierarchical_competition(particles,sequence, i, hierarchy,particle_swarm_hierarchy,phi,epsilon)
    groups = particle_swarm_hierarchy{hierarchy+1};

    random_index = randperm(length(groups), 1);
    group_elements = groups(random_index).groups; 
    random_index = randperm(length(group_elements), 1);
    competitor_index = group_elements(random_index);

    group_mean_position = mean(particles.pos(group_elements, :));
